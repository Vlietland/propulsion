    ...
    private initializeHighScores() {
        ...
        if (savedScores) {
            try {
                ...
                if (Array.isArray(parsed) && parsed.length === 8 && parsed.every(entry => 
                    typeof entry === 'object' && typeof entry.score === 'number' && typeof entry.name === 'string'
                )) {
